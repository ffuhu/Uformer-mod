...
python3 ./train.py \
  ...
  --eval_workers 4 \
  --use_mixup_from_epoch 6 \
  --pretrain_weights ${HOME}/Scratch/nus/20b_deblurring_uformer_nD/log/UNetdeblur_SGgenMVDS_TRIALS_UNet_BAR_bs4_3-1D/models/model_best.pth \
  --resume \
  --reset_optimizer \
  --token _down \
  --data_multiplier 480 \
...
python3 ./train.py \
  --env deblur_SGgenMVDS_TRIALS_UNet_BAR_bs4_3-3D_FT_MVDSmanually_selected_STACK_5_2_bs4_dmult480_ep1500_ps64 \
  --arch UNet \
  --batch_size 4 \
  --gpu ${GPU_ID} \
  --train_ps 64 \
  ...
  --eval_workers 4 \
  --use_mixup_from_epoch 6 \
  --pretrain_weights ${HOME}/Scratch/nus/20b_deblurring_uformer_nD/log/UNetdeblur_SGgenMVDS_TRIALS_UNet_BAR_bs4_3-3D/models/model_best_PSNR_SSIM.pth \
  --resume \
  --reset_optimizer \
  --token _down \
  --data_multiplier 480 \

...
with torch.no_grad():
    ...
    for ii, data_test in enumerate(tqdm(test_loader), 0):
        ...
        rgb_noisy, mask = expand2square(data_test[1].cuda(), factor=128, n_channels=args.in_chans)
        filenames = data_test[2]

        rgb_restored = model_restoration(rgb_noisy, 1 - mask)

        rgb_restored = torch.masked_select(rgb_restored, mask.bool()).reshape(1, args.in_chans, rgb_gt.shape[0], rgb_gt.shape[1])
        rgb_restored = torch.clamp(rgb_restored, 0, 1).cpu().numpy()[0].transpose((1, 2, 0))
        ...
        if args.save_images:
            ...
            rgb_noisy = data_test[1].numpy()[0].transpose((1, 2, 0))
            utils.save_img(os.path.join(args.result_dir, filename_noisy), img_as_ubyte(rgb_noisy))
